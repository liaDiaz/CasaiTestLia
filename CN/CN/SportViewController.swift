...
class SportViewController: UIViewController,UITableViewDataSource, UITableViewDelegate {
    ...
    @IBOutlet weak var endpS: UILabel!
    var endpNS : String = ""
    override func viewDidLoad() {
        super.viewDidLoad()
        self.endpS.text! = endpNS
        
        if let url = URL(string:  self.endpS.text!){
            do{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: "myCellSport", for: indexPath)
        
        if(cell == nil){
            cell=UITableViewCell(style: UITableViewCell.CellStyle.subtitle, reuseIdentifier: "myCellSport")
        }
